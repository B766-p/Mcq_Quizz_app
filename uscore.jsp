...
<%@page import="java.sql.*" %>
   ...
   try
   {
	   ...
	   int i=0;
	  	 Connection con=DBcon.getCon();
	        PreparedStatement ps=con.prepareStatement("select userinfo.uname,resreport.score,resreport.email,resreport.per,resreport.edate from userinfo,resreport where userinfo.email=resreport.email and userinfo.email=?");
	        ps.setString(1, uid);
	        ResultSet rs=ps.executeQuery();
	        while(rs.next())
	        {   i++;
	        	if(rs.getFloat(4)>=85.0)
	        		c="Gold";
	        	else if(rs.getFloat(4)>=60.0 &&rs.getFloat(4)<85.0)
	        		c="Silver";
	        	else if(rs.getFloat(4)<60.0)
	        		c="Bronze";
   ...
   response.setHeader("Refresh", "5;URL=uwelcome.jsp");
   %>
</table>
</body>
</html>